...
| --- | --- | --- |
| 交互式量子化学场 | `10.1109/TVCG.2009.158` 与 `10.1107/S010876731002636X` 直接讨论从高斯基函数表示评估电子密度、静电势、轨道和自旋密度，并使用切片、传递函数、裁剪及体渲染；预采样规则网格会带来存储、传输和分辨率权衡 | 不能把需求简化为“是否打开 Cube”；需测量直接波函数评估、交互阈值/传递函数、裁剪、等值面与体渲染需求 |
| 波函数与实空间分析 | ORBKIT（`10.1002/jcc.24358`）可读取多程序输出并在任意网格重建轨道、密度及导数；Multiwfn（`10.1002/jcc.22885`）覆盖 ESP、ELF、密度、轨道和拓扑分析 | 格式题需同时覆盖 Molden、FCHK、WFN/WFX 等源表示与 Cube、OpenDX、XSF 等体网格；场类型不能只列 HOMO/LUMO |
| Blender 科研表达 | Rhorix（`10.1002/jcc.25054`）将临界点、梯度路径和等值包络映射为 Blender 对象，并指出异构拓扑输出造成转换负担 | Blender 是呈现层而非科学权威；需要保留拓扑语义、对象关系和来源元数据 |
| 稀疏动态体数据 | VDB（`10.1145/2487228.2487235`）面向高分辨率、稀疏、动态拓扑和时间变化体数据，降低稠密包围网格的内存负担 | OpenVDB 适合作为大型、多帧场的派生格式；稀疏化、重采样与误差必须可报告，原始数据仍需保留 |
| 跨程序标准化 | QCDB/QCEngine（`10.1063/5.0059356`）用 QCSchema JSON、验证、单位转换和程序适配器统一输入输出，同时允许程序特定关键字；公共格式论文（`10.1002/qua.21387`）将语义描述与大型数组分别交给 XML/HDF5 | “引擎无关”应包含方法、关键字/默认值、程序版本、单位、数组形状/顺序、原生输入输出及来源关系，不能只看扩展名 |
| 数据可追溯与解析 | QCArchive（`10.1002/wcms.1491`）把完整输入、输出和程序/执行来源共同存档；cclib（`10.1002/jcc.20823`）说明不同程序、版本和任务的文本输出差异会使解析器脆弱 | 派生场应链接到不可变计算记录；解析层需报告适配器版本、支持字段、警告和失败，且保留原始输出 |
| 自动化与采用门槛 | MDI（`10.1016/j.cpc.2020.107688`）规定运行时接口的单位、数据类型、顺序和长度；AQME（`10.1002/wcms.1663`）与 QMflows（`10.1021/acs.jcim.9b00384`）覆盖输入准备、调度、结果收集和多程序工作流；交互量化综述（`10.1146/annurev-physchem-061020-053438`）明确指出领域知识、编程和硬件是进入门槛 | 需求调查必须同时询问 GUI、CLI/Python、HPC/云、批处理、教程和编程门槛；适配器应验证单位、形状、顺序并公开能力边界 |
| 数值可信度 | DFT 教程综述（`10.1002/qua.26332`）指出基组和积分网格常被忽略，默认网格可能不足且应在报告中注明；网格误差研究（`10.1021/ct900489g`）报告 SG-1 下 M06-HF 反应能相对很细网格可出现 −6.7 至 3.2 kcal/mol 的误差；SG-2/SG-3 论文（`10.1063/1.4810804`）给出经系统剪枝和验证的高精度网格 | 可视化验证不能替代上游计算验证；元数据和跨程序对照必须记录网格、基组、版本、默认值覆盖与容差 |
...
问卷将格式拆为两题：多选题记录过去12个月实际接收、生成或处理的格式；单选题要求受访者指出只能优先支持一种时的首选格式。这样既能估计覆盖面，也能给首个适配器明确排序。
...
9. **A perspective on the future of quantum chemical software: the example of the ORCA program package**（2024，Faraday Discussions；DOI `10.1039/D4FD00056K`）  
   ...
   对项目的含义：有助于避免把“Python框架”和“集成程序包”放在同一维度粗暴排名。该条目前未附全文，作为补充背景，不承担问卷核心构念。
...
26. **Interactive Volume Rendering of Functional Representations in Quantum Chemistry**（2009，IEEE TVCG；DOI `10.1109/TVCG.2009.158`）——直接从基函数表示交互评估和体渲染量子化学场，提示不能把需求限定为预采样 `.cube`。
27. **Interactive Visualization of Quantum-Chemistry Data**（2010，Acta Crystallographica A；DOI `10.1107/S010876731002636X`）——讨论传递函数、裁剪和无需重采样的交互探索。
28. **Interactive Quantum Chemistry Enabled by Machine Learning, GPUs, and Cloud Computing**（2023，Annual Review of Physical Chemistry；DOI `10.1146/annurev-physchem-061020-053438`）——从综述层面连接实时计算、云端、交互界面和扩展现实。
29. **Bringing Chemical Structures to Life with Augmented Reality, Machine Learning, and Quantum Chemistry**（2022，JCP；DOI `10.1063/5.0090482`）——说明低门槛沉浸式可视化是一类交付形态。
30. **ORBKIT**（2016，Journal of Computational Chemistry；DOI `10.1002/jcc.24358`）——从多种程序输出读取波函数，在任意网格计算密度、轨道及导数，并输出多种可视化格式。
31. **QCDB and QCEngine**（2021，JCP；DOI `10.1063/5.0059356`）——公共 API 和标准变量可降低多程序输入差异，支撑引擎无关工作流。
32. **The Problem of Interoperability: A Common Data Format for Quantum Chemistry Codes**（2007，IJQC；DOI `10.1002/qua.21387`）——把可读语义和大型二进制数据分别交给 XML 与 HDF5，说明格式设计不只是扩展名兼容。
33. **QMflows**（2019，JCIM；DOI `10.1021/acs.jcim.9b00384`）——展示多程序标准接口、Python 工作流与并行执行的组合。

上述新增条目已写入 Zotero、BibTeX 和工作簿。直接支撑格式、交互可视化、互操作和采用门槛的关键条目均已完成全文核验；QCDB/QCEngine 的替换附件也已确认可提取全文。仅 ORCA 未来软件 Perspective 暂未附全文，已降为非阻塞补充背景，本文不以其摘要独立支撑核心结论。

## 十一、交付物说明

- Excel工作簿：72条文献/官方资料、18个软件条目、30道问卷题、20项验证测试；
- BibTeX：61条带DOI的初筛文献，与Zotero父分类条目数一致；
- 问卷Markdown：12–15分钟文献支撑预测试版，已明确单选/多选，覆盖源波函数格式、体网格格式、首选格式、交互方式、元数据与验证要求；
